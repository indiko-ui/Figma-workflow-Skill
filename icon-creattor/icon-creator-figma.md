...
ก่อน generate script ต้องถาม user 2 อย่างนี้ถ้ายังไม่รู้:
1. **Page name** ที่จะ import เข้า (default: "Icons")
2. **Documentation base URL** — เช่น `https://your-docs.com/icons`
   - ถ้าไม่มี doc site → ให้ใช้ Figma file URL แทน: `https://figma.com/file/{fileKey}`
   - URL slug จะ auto-derive จากชื่อ icon: `icon/home` → `{baseUrl}/home`

**Step 2b — Import icons batch ทีละ 6:**
```js
// ⚠️ แก้ DOC_BASE_URL และ PAGE_NAME ก่อน run เสมอ
const DOC_BASE_URL = "https://your-docs.com/icons";  // ← ต้องใส่จริง
const PAGE_NAME    = "Icons";                          // ← ชื่อ page ใน Figma
...
// docUrl ต้อง generate ทุก icon — ห้ามละ
// slug = ส่วนท้ายของ name: "icon/home" → "home", "icon/nav/home" → "nav/home"
const icons = [
  {
    name: "icon/home",
    description: "Home — navigate to main dashboard",
    docUrl: `${DOC_BASE_URL}/home`,
...
for (const icon of icons) {
  // derive slug จาก name เพื่อ fallback URL ถ้าไม่ได้ระบุ docUrl
  const slug = icon.name.split("/").slice(1).join("/");
  const resolvedUrl = icon.docUrl || `${DOC_BASE_URL}/${slug}`;
  ...
  // description — ห้ามข้าม
  comp.description = icon.description;

  // documentationLinks — set หลัง appendChild เพื่อให้ node อยู่ใน tree ก่อน
  section.appendChild(comp);
  comp.documentationLinks = [{ uri: resolvedUrl }];
...
// verify — อ่านกลับมา confirm ว่า set จริง
...
return { createdNodeIds, verified };
```
...
for (const p of patches) {
  ...
  const slug = p.name.split("/").slice(1).join("/");

  comp.description = p.desc;
  comp.documentationLinks = [{ uri: `${DOC_BASE_URL}/${slug}` }];
